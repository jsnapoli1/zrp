...
func TestDocVersionCRUD(t *testing.T) {
	...
	req := authedRequest("POST", "/api/v1/docs", []byte(`{"title":"Test Doc","category":"spec","content":"Version A content","revision":"A"}`), cookie)
	...
	w = httptest.NewRecorder()
	req = authedRequest("PUT", "/api/v1/docs/"+doc.ID, []byte(`{"title":"Test Doc","category":"spec","content":"Version A updated","revision":"A"}`), cookie)
	...
	// List versions
	w = httptest.NewRecorder()
	req = authedRequest("GET", "/api/v1/docs/"+doc.ID+"/versions", nil, cookie)
...
func TestDocRelease(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("POST", "/api/v1/docs", []byte(`{"title":"Release Test","category":"spec","content":"Draft content","revision":"A","status":"draft"}`), cookie)
	handleCreateDoc(w, req)
	doc := parseDoc(t, w.Body.Bytes())

	// Release
	w = httptest.NewRecorder()
	req = authedRequest("POST", "/api/v1/docs/"+doc.ID+"/release", nil, cookie)
...
func TestDocRevert(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("POST", "/api/v1/docs", []byte(`{"title":"Revert Test","category":"spec","content":"Original content","revision":"A"}`), cookie)
	handleCreateDoc(w, req)
	doc := parseDoc(t, w.Body.Bytes())

	// Update to content B (snapshots A)
	w = httptest.NewRecorder()
	req = authedRequest("PUT", "/api/v1/docs/"+doc.ID, []byte(`{"title":"Revert Test","category":"spec","content":"Updated content","revision":"B"}`), cookie)
	handleUpdateDoc(w, req, doc.ID)

	// Revert to A
	w = httptest.NewRecorder()
	req = authedRequest("POST", "/api/v1/docs/"+doc.ID+"/revert/A", nil, cookie)
...
func TestDocDiff(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("POST", "/api/v1/docs", []byte(`{"title":"Diff Test","category":"spec","content":"Line 1\nLine 2\nLine 3","revision":"A"}`), cookie)
	handleCreateDoc(w, req)
	doc := parseDoc(t, w.Body.Bytes())

	// Update (snapshots revision A)
	w = httptest.NewRecorder()
	req = authedRequest("PUT", "/api/v1/docs/"+doc.ID, []byte(`{"title":"Diff Test","category":"spec","content":"Line 1\nLine 2 modified\nLine 3\nLine 4","revision":"B"}`), cookie)
	handleUpdateDoc(w, req, doc.ID)
	...
	w = httptest.NewRecorder()
	req = authedRequest("GET", "/api/v1/docs/"+doc.ID+"/diff?from=A&to=B", nil, cookie)
...
func TestDocDiffMissingParams(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("GET", "/api/v1/docs/DOC-001/diff", nil, cookie)
...
func TestGetDocVersionByRevision(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("POST", "/api/v1/docs", []byte(`{"title":"Version Get Test","category":"spec","content":"Content A","revision":"A"}`), cookie)
	handleCreateDoc(w, req)
	doc := parseDoc(t, w.Body.Bytes())

	// Update to create version snapshot
	w = httptest.NewRecorder()
	req = authedRequest("PUT", "/api/v1/docs/"+doc.ID, []byte(`{"title":"Version Get Test","category":"spec","content":"Content B","revision":"B"}`), cookie)
	handleUpdateDoc(w, req, doc.ID)

	// Get version A
	w = httptest.NewRecorder()
	req = authedRequest("GET", "/api/v1/docs/"+doc.ID+"/versions/A", nil, cookie)
	...
	w = httptest.NewRecorder()
	req = authedRequest("GET", "/api/v1/docs/"+doc.ID+"/versions/Z", nil, cookie)
...
func TestGitDocsSettings(t *testing.T) {
	...
	// PUT settings
	w := httptest.NewRecorder()
	req := authedRequest("PUT", "/api/v1/settings/git-docs", []byte(`{"repo_url":"https://github.com/test/repo","branch":"main","token":"secret123"}`), cookie)
	...
	w = httptest.NewRecorder()
	req = authedRequest("GET", "/api/v1/settings/git-docs", nil, cookie)
...
func TestPushDocToGitNoConfig(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := authedRequest("POST", "/api/v1/docs/DOC-2026-001/push", nil, cookie)
