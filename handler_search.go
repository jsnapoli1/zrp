...
func handleGlobalSearch(w http.ResponseWriter, r *http.Request) {
	...
	for _, parts := range cats {
		for _, p := range parts {
			if len(matchedParts) >= limit { break }
	...
	if matchedParts == nil { matchedParts = []map[string]string{} }
	...
	var ecos []ecoResult
	ecoRows, _ := db.Query("SELECT id,title,COALESCE(description,''),status FROM ecos")
	if ecoRows != nil {
		defer ecoRows.Close()
		for ecoRows.Next() {
			...
			if strings.Contains(strings.ToLower(id), q) || strings.Contains(strings.ToLower(title), q) || strings.Contains(strings.ToLower(desc), q) {
				...
				if len(ecos) >= limit { break }
			}
		}
	}
	if ecos == nil { ecos = []ecoResult{} }
	total += len(ecos)
	...
	var wos []woResult
	woRows, _ := db.Query("SELECT id,assembly_ipn,status FROM work_orders")
	if woRows != nil {
		defer woRows.Close()
		for woRows.Next() {
			...
			if strings.Contains(strings.ToLower(id), q) || strings.Contains(strings.ToLower(aipn), q) {
				...
				if len(wos) >= limit { break }
			}
		}
	}
	if wos == nil { wos = []woResult{} }
	total += len(wos)
	...
	var devs []devResult
	devRows, _ := db.Query("SELECT serial_number,ipn,COALESCE(customer,''),status FROM devices")
	if devRows != nil {
		defer devRows.Close()
		for devRows.Next() {
	...
	var ncrs []ncrResult
	ncrRows, _ := db.Query("SELECT id,title,status FROM ncrs")
	if ncrRows != nil {
		defer ncrRows.Close()
		for ncrRows.Next() {
			...
			if strings.Contains(strings.ToLower(id), q) || strings.Contains(strings.ToLower(title), q) {
				...
				if len(ncrs) >= limit { break }
			}
		}
	}
	if ncrs == nil { ncrs = []ncrResult{} }
	total += len(ncrs)
	...
	var pos []poResult
	poRows, _ := db.Query("SELECT id,status FROM purchase_orders")
	if poRows != nil {
		defer poRows.Close()
		for poRows.Next() {
	...
	qRows, _ := db.Query("SELECT id,COALESCE(customer,''),status FROM quotes")
	if qRows != nil {
		defer qRows.Close()
		for qRows.Next() {
			...
			if strings.Contains(strings.ToLower(id), q) || strings.Contains(strings.ToLower(cust), q) {
				...
				if len(quotes) >= limit { break }
			}
		}
	}
	if quotes == nil { quotes = []quoteResult{} }
	total += len(quotes)
