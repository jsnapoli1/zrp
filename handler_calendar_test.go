...
func TestHandleCalendar_CurrentMonth(t *testing.T) {
	...
	if len(resp.Data) != 3 {
		t.Errorf("Expected 3 events for current month, got %d", len(resp.Data))
	}
...
func TestHandleCalendar_SpecificMonth(t *testing.T) {
	...
	if len(resp.Data) != 1 {
		t.Errorf("Expected 1 event for March 2026, got %d", len(resp.Data))
	}

	if len(resp.Data) > 0 && resp.Data[0].ID != "WO-MARCH" {
		t.Errorf("Expected WO-MARCH, got %s", resp.Data[0].ID)
	}
}
...
func TestHandleCalendar_Empty(t *testing.T) {
	...
	if len(resp.Data) != 0 {
		t.Errorf("Expected empty array for month with no events, got %d", len(resp.Data))
	}
}
...
func TestHandleCalendar_WorkOrderWithNotes(t *testing.T) {
	...
	if resp.Data[0].Title != "Custom build for customer" {
		t.Errorf("Expected title to use notes, got '%s'", resp.Data[0].Title)
	}
}
...
func TestHandleCalendar_WorkOrderWithoutNotes(t *testing.T) {
	...
	if resp.Data[0].Title != expected {
		t.Errorf("Expected title '%s', got '%s'", expected, resp.Data[0].Title)
	}
}
...
func TestHandleCalendar_WorkOrderEstimatedDueDate(t *testing.T) {
	...
	if len(resp.Data) != 1 {
		t.Errorf("Expected 1 event for estimated due date in March, got %d", len(resp.Data))
	}

	if len(resp.Data) > 0 && resp.Data[0].ID != "WO-ESTIMATED" {
		t.Errorf("Expected WO-ESTIMATED, got %s", resp.Data[0].ID)
	}
}
...
func TestHandleCalendar_QuoteWithCustomer(t *testing.T) {
	...
	for i := range resp.Data {
		if resp.Data[i].Type == "quote" {
			quoteEvent = &resp.Data[i]
			break
		}
	}
...
func TestHandleCalendar_DateFormatting(t *testing.T) {
	...
	if resp.Data[0].Date != expectedDate {
		t.Errorf("Expected date '%s', got '%s'", expectedDate, resp.Data[0].Date)
	}
}
...
func TestHandleCalendar_DateRangeQuery(t *testing.T) {
	...
	if len(resp.Data) != 3 {
		t.Errorf("Expected 3 events in March, got %d", len(resp.Data))
	}
...
func TestHandleCalendar_MultipleEventsPerDay(t *testing.T) {
	...
	if len(resp.Data) != 3 {
		t.Errorf("Expected 3 events on same day, got %d", len(resp.Data))
	}
...
func TestHandleCalendar_InvalidMonth(t *testing.T) {
	...
	if len(resp.Data) != 0 {
		t.Logf("Got %d events for month 13 (handled by SQLite)", len(resp.Data))
	}
}
...
func TestHandleCalendar_EventAggregation(t *testing.T) {
	...
	if len(resp.Data) != 18 {
		t.Errorf("Expected 18 total events (10 WO + 5 PO + 3 quotes), got %d", len(resp.Data))
	}
...
func TestHandleCalendar_DefaultsToCurrentMonth(t *testing.T) {
	...
	if len(resp.Data) != 1 {
		t.Errorf("Expected 1 event in current month, got %d", len(resp.Data))
	}
}
