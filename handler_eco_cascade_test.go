...
func setupECOCascadeTestEnv(t *testing.T) (*sql.DB, string, func()) {
	...
	return testDB, partsSubDir, cleanup
}

// createTestPart creates a part CSV file with the specified fields
func createTestPart(t *testing.T, partsDir, ipn, revision, description string, additionalFields map[string]string) {
...
// createTestBOM creates a BOM CSV file for an assembly
func createTestBOM(t *testing.T, partsDir, assemblyIPN string, components []BOMComponent) {
...
// BOMComponent represents a component in a BOM
...
// readBOMComponents reads components from a BOM CSV file
func readBOMComponents(t *testing.T, partsDir, assemblyIPN string) []BOMComponent {
...
// getPartRevision reads the revision field from a part's CSV record
func getPartRevision(t *testing.T, partsDir, ipn string) string {
...
// updatePartRevisionInCSV updates a part's revision in the CSV file
func updatePartRevisionInCSV(partsDir, ipn, newRevision string) error {
	csvPath := filepath.Join(partsDir, "components", "components.csv")
	f, err := os.Open(csvPath)
	if err != nil {
		return err
...
// updateBOMReferences updates all references to a part in BOM files
func updateBOMReferencesForPart(partsDir, oldIPN, newIPN string) error {
	bomDir := filepath.Join(partsDir, "assemblies")
	entries, err := os.ReadDir(bomDir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil // No BOMs to update
		}
		return err
	}
...
func TestECOPartRevisionCascade(t *testing.T) {
	...
	testDB, testPartsDir, cleanup := setupECOCascadeTestEnv(t)
	defer cleanup()
	defer func() {
		db = oldDB
	...
	createTestPart(t, partsDir, "RES-100", "1.0", "1k Resistor 0603", map[string]string{
		"MPN":          "RC0603FR-071KL",
		"Manufacturer": "Yageo",
		"Type":         "Resistor",
	})

	// Verify part was created with v1.0
	rev := getPartRevision(t, partsDir, "RES-100")
	if rev != "1.0" {
	...
	createTestPart(t, partsDir, "PCA-001", "A", "Main Board Assembly", map[string]string{
		"Type": "Assembly",
	})

	createTestBOM(t, partsDir, "PCA-001", []BOMComponent{
		{IPN: "RES-100", Qty: 10, RefDes: "R1-R10", Description: "1k Resistor"},
		{IPN: "CAP-200", Qty: 5, RefDes: "C1-C5", Description: "10uF Cap"},
	})

	// Verify BOM contains RES-100
	bomComponents := readBOMComponents(t, partsDir, "PCA-001")
	...
	var status string
	db.QueryRow("SELECT status FROM ecos WHERE id=?", ecoID).Scan(&status)
	if status != "approved" {
	...
	// First, manually update the part revision to simulate ECO implementation
	if err := updatePartRevisionInCSV(partsDir, "RES-100", "1.1"); err != nil {
		t.Fatalf("Failed to update part revision: %v", err)
	}

	// Now test the BOM cascade logic
	if err := updateBOMReferencesForPart(partsDir, "RES-100", "RES-100"); err != nil {
	...
	newRev := getPartRevision(t, partsDir, "RES-100")
	if newRev != "1.1" {
		t.Errorf("Expected part revision 1.1 after ECO implementation, got %s", newRev)
	}

	// Step 7: Verify BOM still references RES-100 (revision is in part record, not BOM)
	t.Log("Step 7: Verifying BOM references remain intact")
	updatedBOM := readBOMComponents(t, partsDir, "PCA-001")
	if len(updatedBOM) != 2 {
	...
	var auditCount int
	db.QueryRow("SELECT COUNT(*) FROM audit_log WHERE record_id=? AND module='eco'", ecoID).Scan(&auditCount)
	if auditCount < 3 {
	...
	var changeCount int
	db.QueryRow("SELECT COUNT(*) FROM part_changes WHERE record_id=?", ecoID).Scan(&changeCount)
	if changeCount < 1 {
...
func TestECOMultipleBOMCascade(t *testing.T) {
	...
	createTestPart(t, partsDir, "CAP-300", "1.0", "100uF Capacitor", map[string]string{
		"Type": "Capacitor",
	})

	// Create multiple assemblies using this component
	t.Log("Creating multiple assemblies using CAP-300")
	createTestPart(t, partsDir, "PCA-100", "A", "Power Supply Board", map[string]string{"Type": "Assembly"})
	createTestPart(t, partsDir, "PCA-200", "A", "Control Board", map[string]string{"Type": "Assembly"})
	createTestPart(t, partsDir, "ASY-001", "A", "Main Assembly", map[string]string{"Type": "Assembly"})

	createTestBOM(t, partsDir, "PCA-100", []BOMComponent{
		{IPN: "CAP-300", Qty: 4, RefDes: "C1-C4", Description: "100uF Cap"},
	})
	createTestBOM(t, partsDir, "PCA-200", []BOMComponent{
		{IPN: "CAP-300", Qty: 2, RefDes: "C10-C11", Description: "100uF Cap"},
	})
	createTestBOM(t, partsDir, "ASY-001", []BOMComponent{
		{IPN: "CAP-300", Qty: 1, RefDes: "C20", Description: "100uF Cap"},
	})

	// Update part to v2.0
	t.Log("Updating CAP-300 to v2.0")
	if err := updatePartRevisionInCSV(partsDir, "CAP-300", "2.0"); err != nil {
	...
	for _, asm := range assemblies {
		components := readBOMComponents(t, partsDir, asm)
		found := false
		for _, comp := range components {
			if comp.IPN == "CAP-300" {
				found = true
	...
	newRev := getPartRevision(t, partsDir, "CAP-300")
	if newRev != "2.0" {
...
func TestECORevisionHistoryPreservation(t *testing.T) {
	...
	// Create part
	createTestPart(t, partsDir, "MCU-500", "1.0", "Microcontroller", map[string]string{
		"Type": "IC",
	})
	...
	updatePartRevisionInCSV(partsDir, "MCU-500", "1.1")
	...
	var revCount int
	db.QueryRow("SELECT COUNT(*) FROM eco_revisions WHERE eco_id=?", ecoID).Scan(&revCount)
	if revCount < 1 {
	...
	db.QueryRow("SELECT status FROM eco_revisions WHERE eco_id=? ORDER BY id DESC LIMIT 1", ecoID).Scan(&implStatus)
