...
// InitDB opens a SQLite connection with WAL mode and foreign keys.
// If prev is non-nil it is closed first to prevent goroutine leaks.
func InitDB(prev *sql.DB, path string) (*sql.DB, error) {
	if prev != nil {
		prev.Close()
	}
	sep := "?"
...
// RunMigrations creates all tables, indexes, and runs ALTER TABLE migrations.
// searchTableInit is a callback for InitSearchTables (lives outside this package).
func RunMigrations(db *sql.DB, searchTableInit func(*sql.DB) error) error {
	...
	tables := []string{
		...
		`CREATE TABLE IF NOT EXISTS change_history (
			...
			operation TEXT NOT NULL, old_data TEXT, new_data TEXT,
			user_id TEXT NOT NULL,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP, undone INTEGER DEFAULT 0
		)`,
		...
		`CREATE TABLE IF NOT EXISTS password_history (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			user_id INTEGER NOT NULL, password_hash TEXT NOT NULL,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
		)`,
		...
		`CREATE TABLE IF NOT EXISTS audit_log (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			user_id INTEGER, username TEXT DEFAULT 'system',
			action TEXT NOT NULL, module TEXT NOT NULL,
			record_id TEXT NOT NULL, summary TEXT,
		...
		`CREATE TABLE IF NOT EXISTS api_keys (
			...
			key_prefix TEXT NOT NULL, created_by TEXT DEFAULT 'admin',
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			last_used DATETIME, expires_at DATETIME, enabled INTEGER DEFAULT 1
		)`,
		`CREATE TABLE IF NOT EXISTS attachments (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			module TEXT NOT NULL, record_id TEXT NOT NULL,
			filename TEXT NOT NULL, original_name TEXT NOT NULL,
			size_bytes INTEGER, mime_type TEXT, uploaded_by TEXT,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP
		)`,
		`CREATE TABLE IF NOT EXISTS notifications (
			...
			record_id TEXT, module TEXT,
			user_id TEXT DEFAULT '', emailed INTEGER DEFAULT 0,
			read_at DATETIME,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP
		)`,
		`CREATE TABLE IF NOT EXISTS price_history (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			ipn TEXT NOT NULL, vendor_id TEXT, vendor_name TEXT,
			unit_price REAL NOT NULL, currency TEXT DEFAULT 'USD',
			min_qty INTEGER DEFAULT 1, lead_time_days INTEGER,
			po_id TEXT, recorded_at DATETIME DEFAULT CURRENT_TIMESTAMP, notes TEXT
		)`,
		`CREATE TABLE IF NOT EXISTS email_config (
			id INTEGER PRIMARY KEY DEFAULT 1,
			smtp_host TEXT, smtp_port INTEGER DEFAULT 587,
			smtp_user TEXT, smtp_password TEXT,
			from_address TEXT, from_name TEXT DEFAULT 'ZRP', enabled INTEGER DEFAULT 0
		)`,
		`CREATE TABLE IF NOT EXISTS email_log (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			to_address TEXT NOT NULL, recipient TEXT DEFAULT '',
		...
		`CREATE TABLE IF NOT EXISTS email_subscriptions (
			...
			enabled INTEGER DEFAULT 1, UNIQUE(user_id, event_type)
		)`,
		`CREATE TABLE IF NOT EXISTS eco_revisions (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			eco_id TEXT NOT NULL, revision TEXT NOT NULL DEFAULT 'A',
			status TEXT NOT NULL DEFAULT 'created', changes_summary TEXT,
			created_by TEXT,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			approved_by TEXT, approved_at DATETIME,
			implemented_by TEXT, implemented_at DATETIME,
			effectivity_date TEXT, notes TEXT,
			FOREIGN KEY (eco_id) REFERENCES ecos(id)
		)`,
		`CREATE TABLE IF NOT EXISTS dashboard_widgets (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			user_id INTEGER DEFAULT 0, widget_type TEXT NOT NULL,
			position INTEGER DEFAULT 0, enabled INTEGER DEFAULT 1
		)`,
		`CREATE TABLE IF NOT EXISTS receiving_inspections (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			po_id TEXT NOT NULL, po_line_id INTEGER NOT NULL,
			ipn TEXT NOT NULL,
			...
			inspector TEXT, inspected_at DATETIME, notes TEXT,
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
		...
		`CREATE TABLE IF NOT EXISTS rfq_vendors (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			rfq_id TEXT NOT NULL, vendor_id TEXT NOT NULL,
			status TEXT DEFAULT 'pending' CHECK(status IN ('pending','quoted','declined','awarded')),
			quoted_at DATETIME, notes TEXT,
			FOREIGN KEY (rfq_id) REFERENCES rfqs(id) ON DELETE CASCADE,
			FOREIGN KEY (vendor_id) REFERENCES vendors(id) ON DELETE RESTRICT
		)`,
		`CREATE TABLE IF NOT EXISTS rfq_lines (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			rfq_id TEXT NOT NULL, ipn TEXT NOT NULL,
			description TEXT, qty REAL NOT NULL DEFAULT 0 CHECK(qty >= 0),
			unit TEXT DEFAULT 'ea',
			FOREIGN KEY (rfq_id) REFERENCES rfqs(id) ON DELETE CASCADE
		)`,
		`CREATE TABLE IF NOT EXISTS rfq_quotes (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			rfq_id TEXT NOT NULL, rfq_vendor_id INTEGER NOT NULL,
			rfq_line_id INTEGER NOT NULL,
			unit_price REAL DEFAULT 0 CHECK(unit_price >= 0),
			lead_time_days INTEGER DEFAULT 0 CHECK(lead_time_days >= 0),
			moq INTEGER DEFAULT 0 CHECK(moq >= 0), notes TEXT,
			FOREIGN KEY (rfq_id) REFERENCES rfqs(id) ON DELETE CASCADE,
		...
		`CREATE TABLE IF NOT EXISTS cost_analysis (
			...
			bom_cost REAL DEFAULT 0, labor_cost REAL DEFAULT 0,
			overhead_cost REAL DEFAULT 0, total_cost REAL DEFAULT 0,
		...
		`CREATE TABLE IF NOT EXISTS document_versions (
			...
			content TEXT DEFAULT '', file_path TEXT DEFAULT '',
			change_summary TEXT DEFAULT '', status TEXT DEFAULT 'draft',
		...
		`CREATE TABLE IF NOT EXISTS market_pricing (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			part_ipn TEXT NOT NULL, mpn TEXT NOT NULL,
			distributor TEXT NOT NULL, distributor_pn TEXT DEFAULT '',
			manufacturer TEXT DEFAULT '', description TEXT DEFAULT '',
			stock_qty INTEGER DEFAULT 0, lead_time_days INTEGER DEFAULT 0,
			currency TEXT DEFAULT 'USD', price_breaks TEXT DEFAULT '[]',
			product_url TEXT DEFAULT '', datasheet_url TEXT DEFAULT '',
	...
	tables = append(tables, `CREATE TABLE IF NOT EXISTS parts (
		ipn TEXT PRIMARY KEY, category TEXT DEFAULT '',
		description TEXT DEFAULT '', mpn TEXT DEFAULT '',
		manufacturer TEXT DEFAULT '', lifecycle TEXT DEFAULT 'active',
	...
	tables = append(tables, `CREATE TABLE IF NOT EXISTS part_changes (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		part_ipn TEXT NOT NULL, eco_id TEXT DEFAULT '',
		field_name TEXT NOT NULL, old_value TEXT DEFAULT '',
		new_value TEXT DEFAULT '', status TEXT DEFAULT 'draft',
		created_by TEXT DEFAULT '',
		created_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`)
	tables = append(tables, `CREATE TABLE IF NOT EXISTS sales_orders (
		id TEXT PRIMARY KEY, quote_id TEXT DEFAULT '',
	...
	tables = append(tables, `CREATE TABLE IF NOT EXISTS sales_order_lines (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		sales_order_id TEXT NOT NULL, ipn TEXT NOT NULL,
	...
	tables = append(tables, `CREATE TABLE IF NOT EXISTS invoices (
		...
		sales_order_id TEXT NOT NULL, customer TEXT NOT NULL,
		issue_date DATE NOT NULL, due_date DATE NOT NULL,
		status TEXT DEFAULT 'draft' CHECK(status IN ('draft','sent','paid','overdue','cancelled')),
		total REAL DEFAULT 0, tax REAL DEFAULT 0,
		notes TEXT DEFAULT '',
	...
	tables = append(tables, `CREATE TABLE IF NOT EXISTS invoice_lines (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		invoice_id TEXT NOT NULL, ipn TEXT NOT NULL DEFAULT '',
	...
	if searchTableInit != nil {
		if err := searchTableInit(db); err != nil {
...
// SeedDB populates default data (users, email config, widgets, demo data).
...
// NextID generates a sequential ID like "ECO-2026-001".
func NextID(db *sql.DB, prefix string, table string, digits int) string {
	year := time.Now().Format("2006")
	pattern := prefix + "-" + year + "-%"
...
// NS converts a *string to sql.NullString.
...
// SP converts a sql.NullString to *string.
