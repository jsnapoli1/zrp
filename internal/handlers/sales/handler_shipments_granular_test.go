...
// extractShipment decodes a single Shipment from the APIResponse envelope.
func extractShipment(t *testing.T, body []byte) models.Shipment {
	t.Helper()
...
// extractShipments decodes a slice of Shipments from the APIResponse envelope.
func extractShipments(t *testing.T, body []byte) []models.Shipment {
	t.Helper()
	var resp models.APIResponse
	json.Unmarshal(body, &resp)
	b, _ := json.Marshal(resp.Data)
...
// extractPackList decodes a PackList from the APIResponse envelope.
func extractPackList(t *testing.T, body []byte) models.PackList {
	t.Helper()
	var resp models.APIResponse
	json.Unmarshal(body, &resp)
	b, _ := json.Marshal(resp.Data)
	var pl models.PackList
	json.Unmarshal(b, &pl)
	return pl
}
...
func TestHandleListShipments(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			response := extractShipments(t, w.Body.Bytes())
...
func TestHandleGetShipment(t *testing.T) {
	...
	h.GetShipment(w, req, "SHP-0001")

	if w.Code != 200 {
		t.Fatalf("Expected status 200, got %d", w.Code)
	}

	response := extractShipment(t, w.Body.Bytes())
...
func TestHandleUpdateShipment(t *testing.T) {
	...
	h.UpdateShipment(w, req, "SHP-0001")
...
func TestHandleDeliverShipment(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			h.DeliverShipment(w, req, "SHP-0001")
...
func TestHandleShipmentPackList(t *testing.T) {
	...
	h.ShipmentPackList(w, req, "SHP-0001")

	if w.Code != 200 {
		t.Fatalf("Expected status 200, got %d", w.Code)
	}

	response := extractPackList(t, w.Body.Bytes())
...
func TestHandleShipmentPackList_NotFound(t *testing.T) {
	...
	h.ShipmentPackList(w, req, "SHP-9999")
...
// Test concurrent-style shipment operations (sequential, verifying idempotency)
func TestShipment_ConcurrentOperations(t *testing.T) {
	...
	// Try to ship the same shipment multiple times sequentially.
	// The first should succeed, subsequent attempts should fail gracefully.
	successCount := 0
	for i := 0; i < 5; i++ {
		input := map[string]string{
			"tracking_number": "TRACK" + string(rune('0'+i)),
			"carrier":         "FedEx",
		}
