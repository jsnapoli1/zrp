...
// Test helpers
func extractInvoice(t *testing.T, body []byte) models.Invoice {
	t.Helper()
...
func extractInvoices(t *testing.T, body []byte) []models.Invoice {
	t.Helper()
	var resp models.APIResponse
	json.Unmarshal(body, &resp)
	b, _ := json.Marshal(resp.Data)
...
func setupInvoiceTestData(t *testing.T, db *sql.DB, h interface{ CreateQuote(http.ResponseWriter, *http.Request) }) (salesOrderID, quoteID string) {
	t.Helper()
	now := time.Now()

	// Create test quote
	quoteID = fmt.Sprintf("Q-%d-001", now.Year())
	_, err := db.Exec(`INSERT INTO quotes (id, customer, status, created_at) VALUES (?, ?, ?, ?)`,
		quoteID, "Test Customer", "accepted", now.Format(time.RFC3339))
	if err != nil {
	...
	salesOrderID = fmt.Sprintf("SO-%d-001", now.Year())
	_, err = db.Exec(`INSERT INTO sales_orders (id, quote_id, customer, status, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		salesOrderID, quoteID, "Test Customer", "shipped", "testuser", now.Format(time.RFC3339), now.Format(time.RFC3339))
	if err != nil {
...
func TestCreateInvoiceAlreadyExists(t *testing.T) {
	...
	invoiceID := fmt.Sprintf("INV-%d-000001", time.Now().Year())
...
func TestGetInvoice(t *testing.T) {
	...
	now := time.Now()
	salesOrderID := fmt.Sprintf("SO-%d-001", now.Year())
	invoiceID := fmt.Sprintf("INV-%d-000001", now.Year())
	...
	h.GetInvoice(rec, req, invoiceID)
...
func TestSendInvoice(t *testing.T) {
	...
	now := time.Now()
	invoiceID := fmt.Sprintf("INV-%d-000001", now.Year())
	soID := fmt.Sprintf("SO-%d-001", now.Year())
	...
	h.SendInvoice(rec, req, invoiceID)
...
func TestMarkInvoicePaid(t *testing.T) {
	...
	h.MarkInvoicePaid(rec, req, invoiceID)
...
func TestInvoicePDFGeneration(t *testing.T) {
	...
	h.GenerateInvoicePDF(rec, req, invoiceID)
