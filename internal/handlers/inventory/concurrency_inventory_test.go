...
func newConcurrencyTestHandler(db *sql.DB) *inventory.Handler {
...
func TestConcurrentInventoryUpdates_TwoGoroutines(t *testing.T) {
	testDB := setupConcurrencyTestDB(t)
	defer testDB.Close()
	h := newConcurrencyTestHandler(testDB)

	// Create initial inventory with qty=100
	_, err := testDB.Exec(`INSERT INTO inventory (ipn, qty_on_hand) VALUES ('IPN-CONCURRENT-1', 100)`)
	if err != nil {
	...
	err = testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-CONCURRENT-1").Scan(&finalQty)
	if err != nil {
	...
	var txCount int
	testDB.QueryRow("SELECT COUNT(*) FROM inventory_transactions WHERE ipn=?", "IPN-CONCURRENT-1").Scan(&txCount)
	if txCount != 2 {
...
func TestConcurrentInventoryUpdates_TenGoroutines(t *testing.T) {
	...
	// Create initial inventory with qty=100
	_, err := testDB.Exec(`INSERT INTO inventory (ipn, qty_on_hand) VALUES ('IPN-CONCURRENT-10', 100)`)
	if err != nil {
		t.Fatalf("Failed to insert test data: %v", err)
	}
	...
	var finalQty float64
	err = testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-CONCURRENT-10").Scan(&finalQty)
	if err != nil {
		t.Fatalf("Failed to query final quantity: %v", err)
	}
	...
	var txCount int
	testDB.QueryRow("SELECT COUNT(*) FROM inventory_transactions WHERE ipn=?", "IPN-CONCURRENT-10").Scan(&txCount)
...
func TestConcurrentInventoryUpdates_DifferentParts(t *testing.T) {
	...
	_, err := testDB.Exec(`INSERT INTO inventory (ipn, qty_on_hand) VALUES
		('IPN-PART-A', 100),
		('IPN-PART-B', 200)
	`)
	if err != nil {
	...
	testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-PART-A").Scan(&qtyA)
	testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-PART-B").Scan(&qtyB)
	...
	testDB.QueryRow("SELECT COUNT(*) FROM inventory_transactions WHERE ipn=?", "IPN-PART-A").Scan(&txCountA)
	testDB.QueryRow("SELECT COUNT(*) FROM inventory_transactions WHERE ipn=?", "IPN-PART-B").Scan(&txCountB)
...
func TestConcurrentInventoryUpdates_MixedOperations(t *testing.T) {
	...
	_, err := testDB.Exec(`INSERT INTO inventory (ipn, qty_on_hand) VALUES ('IPN-MIXED', 1000)`)
	if err != nil {
		t.Fatalf("Failed to insert test data: %v", err)
	}
	...
	var finalQty float64
	err = testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-MIXED").Scan(&finalQty)
	if err != nil {
		t.Fatalf("Failed to query final quantity: %v", err)
	}
	...
	var txCount int
	testDB.QueryRow("SELECT COUNT(*) FROM inventory_transactions WHERE ipn=?", "IPN-MIXED").Scan(&txCount)
	if txCount != 15 {
...
func TestConcurrentInventoryRead_WhileUpdating(t *testing.T) {
	...
	_, err := testDB.Exec(`INSERT INTO inventory (ipn, qty_on_hand) VALUES ('IPN-READ-TEST', 500)`)
	if err != nil {
		t.Fatalf("Failed to insert test data: %v", err)
	}
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx int) {
			...
			w := httptest.NewRecorder()
			h.GetInventory(w, req, "IPN-READ-TEST")
	...
	// Verify final state
	var finalQty float64
	testDB.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "IPN-READ-TEST").Scan(&finalQty)
