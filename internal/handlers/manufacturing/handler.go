...
// Handler holds dependencies for manufacturing/work-order handlers.
type Handler struct {
	DB               *sql.DB
	Hub              *websocket.Hub
	PartsDir         string
	...
	// GetWorkOrderSnapshot returns a snapshot of a work order row for change tracking.
	GetWorkOrderSnapshot func(id string) (map[string]interface{}, error)

	// EmailOnOverdueWorkOrder sends an email notification for overdue work orders.
	EmailOnOverdueWorkOrder func(woID string)
}

// ListWorkOrders handles GET /api/workorders.
func (h *Handler) ListWorkOrders(w http.ResponseWriter, r *http.Request) {
	rows, err := h.DB.Query("SELECT id,assembly_ipn,qty,qty_good,qty_scrap,status,priority,COALESCE(notes,''),created_at,started_at,completed_at FROM work_orders ORDER BY created_at DESC")
	if err != nil {
	...
	for rows.Next() {
		...
		wo.StartedAt = database.SP(sa)
		wo.CompletedAt = database.SP(ca)
		if qtyGood.Valid {
			good := int(qtyGood.Int64)
			wo.QtyGood = &good
		}
		if qtyScrap.Valid {
			scrap := int(qtyScrap.Int64)
			wo.QtyScrap = &scrap
		}
...
// GetWorkOrder handles GET /api/workorders/:id.
func (h *Handler) GetWorkOrder(w http.ResponseWriter, r *http.Request, id string) {
	var wo models.WorkOrder
	var sa, ca sql.NullString
	var qtyGood, qtyScrap sql.NullInt64
	err := h.DB.QueryRow("SELECT id,assembly_ipn,qty,qty_good,qty_scrap,status,priority,COALESCE(notes,''),created_at,started_at,completed_at FROM work_orders WHERE id=?", id).
...
// CreateWorkOrder handles POST /api/workorders.
func (h *Handler) CreateWorkOrder(w http.ResponseWriter, r *http.Request) {
	var wo models.WorkOrder
	if err := response.DecodeBody(r, &wo); err != nil {
		response.Err(w, "invalid body", 400)
		return
	}

	ve := &validation.ValidationErrors{}
	validation.RequireField(ve, "assembly_ipn", wo.AssemblyIPN)
	validation.ValidateMaxLength(ve, "assembly_ipn", wo.AssemblyIPN, 100)
	validation.ValidateMaxLength(ve, "notes", wo.Notes, 10000)
	if wo.Status != "" {
		validation.ValidateEnum(ve, "status", wo.Status, validation.ValidWOStatuses)
	}
	if wo.Priority != "" {
		validation.ValidateEnum(ve, "priority", wo.Priority, validation.ValidWOPriorities)
	}
	if wo.Qty < 0 {
		ve.Add("qty", "must be non-negative")
	}
	validation.ValidateIntRange(ve, "qty", wo.Qty, 1, validation.MaxWorkOrderQty)
	if ve.HasErrors() {
		response.Err(w, ve.Error(), 400)
		return
	}

	wo.ID = h.NextIDFunc("WO", "work_orders", 4)
	if wo.Status == "" {
		wo.Status = "open"
	}
	if wo.Priority == "" {
	...
	_, err := h.DB.Exec("INSERT INTO work_orders (id,assembly_ipn,qty,qty_good,qty_scrap,status,priority,notes,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
	...
	audit.LogAudit(h.DB, h.Hub, username, "created", "workorder", wo.ID, "Created WO "+wo.ID+" for "+wo.AssemblyIPN)
	h.RecordChangeJSON(username, "work_orders", wo.ID, "create", nil, wo)
	response.JSON(w, wo)
}

// UpdateWorkOrder handles PUT /api/workorders/:id.
func (h *Handler) UpdateWorkOrder(w http.ResponseWriter, r *http.Request, id string) {
	oldSnap, _ := h.GetWorkOrderSnapshot(id)
	var wo models.WorkOrder
	if err := response.DecodeBody(r, &wo); err != nil {
		response.Err(w, "invalid body", 400)
		return
	...
	if qtyGood.Valid {
		good := int(qtyGood.Int64)
		currentWO.QtyGood = &good
	}
	if qtyScrap.Valid {
		scrap := int(qtyScrap.Int64)
		currentWO.QtyScrap = &scrap
	}
	...
	if wo.QtyGood != nil && *wo.QtyGood < 0 {
		ve.Add("qty_good", "must be non-negative")
	}
	if wo.QtyScrap != nil && *wo.QtyScrap < 0 {
		ve.Add("qty_scrap", "must be non-negative")
	}
	if ve.HasErrors() {
		response.Err(w, ve.Error(), 400)
		return
	...
	if wo.Status == "completed" && currentWO.Status != "completed" {
		err = HandleWorkOrderCompletion(tx, id, wo.AssemblyIPN, wo.Qty, username)
		if err != nil {
			response.Err(w, "failed to update inventory on completion: "+err.Error(), 500)
			return
		}
	}
	...
	if wo.Status == "cancelled" && currentWO.Status != "cancelled" {
		...
		if err != nil {
			response.Err(w, "failed to release inventory on cancellation: "+err.Error(), 500)
			return
		}
	}
	...
	audit.LogAudit(h.DB, h.Hub, username, "updated", "workorder", id, "Updated WO "+id+": status="+wo.Status)
	newSnap, _ := h.GetWorkOrderSnapshot(id)
	h.RecordChangeJSON(username, "work_orders", id, "update", oldSnap, newSnap)
	if h.EmailOnOverdueWorkOrder != nil {
		go h.EmailOnOverdueWorkOrder(id)
	}
	h.GetWorkOrder(w, r, id)
}

// IsValidStatusTransition defines valid state machine transitions for work orders.
...
// HandleWorkOrderCompletion handles inventory updates when a work order is completed.
func HandleWorkOrderCompletion(tx *sql.Tx, woID, assemblyIPN string, qty int, username string) error {
	...
	// 2. Deduct consumed materials based on BOM (simplified)
...
// HandleWorkOrderCancellation releases reserved inventory when a work order is cancelled.
...
// WorkOrderBOM handles GET /api/workorders/:id/bom.
func (h *Handler) WorkOrderBOM(w http.ResponseWriter, r *http.Request, id string) {
	var assemblyIPN string
	var qty int
	err := h.DB.QueryRow("SELECT assembly_ipn,qty FROM work_orders WHERE id=?", id).Scan(&assemblyIPN, &qty)
	if err != nil {
		response.Err(w, "not found", 404)
		return
	}
	...
	rows, _ := h.DB.Query(`SELECT ipn, qty_on_hand FROM inventory
	...
	if rows != nil {
		...
		for rows.Next() {
			...
			if h.GetPartByIPN != nil {
				fields, err := h.GetPartByIPN(h.PartsDir, bl.IPN)
				if err == nil {
	...
	response.JSON(w, map[string]interface{}{"wo_id": id, "assembly_ipn": assemblyIPN, "qty": qty, "bom": bom})
}

// WorkOrderPDF handles GET /api/workorders/:id/pdf.
func (h *Handler) WorkOrderPDF(w http.ResponseWriter, r *http.Request, id string) {
	var wo models.WorkOrder
	var sa, ca sql.NullString
	err := h.DB.QueryRow("SELECT id,assembly_ipn,qty,status,priority,COALESCE(notes,''),created_at,started_at,completed_at FROM work_orders WHERE id=?", id).
	...
	if rows != nil {
		...
		for rows.Next() {
			...
			if h.GetPartByIPN != nil {
				fields, ferr := h.GetPartByIPN(h.PartsDir, bl.IPN)
				if ferr == nil {
					for k, v := range fields {
	...
	assemblyDesc := ""
	if h.GetPartByIPN != nil {
		if fields, ferr := h.GetPartByIPN(h.PartsDir, wo.AssemblyIPN); ferr == nil {
			for k, v := range fields {
				if strings.EqualFold(k, "description") || strings.EqualFold(k, "desc") {
					assemblyDesc = v
					break
...
<div class="header">
  <div class="header-left">
    ...
    <div style="font-size:10pt;color:#555">`+h.CompanyName+`</div>
  </div>
...
// WorkOrderKit handles POST /api/workorders/:id/kit.
func (h *Handler) WorkOrderKit(w http.ResponseWriter, r *http.Request, id string) {
	var assemblyIPN string
	var qty int
	var status string
	err := h.DB.QueryRow("SELECT assembly_ipn,qty,status FROM work_orders WHERE id=?", id).Scan(&assemblyIPN, &qty, &status)
	if err != nil {
		response.Err(w, "work order not found", 404)
		return
	}
	...
	if status == "completed" || status == "cancelled" {
		response.Err(w, "cannot kit materials for completed or cancelled work order", 400)
		return
	}
	...
	rows, err := h.DB.Query("SELECT ipn, qty_on_hand, qty_reserved FROM inventory")
	if err != nil {
		response.Err(w, err.Error(), 500)
		return
	}
	...
	audit.LogAudit(h.DB, h.Hub, audit.GetUsername(h.DB, r), "kitted", "workorder", id, "Kitted materials for WO "+id)
...
// WorkOrderSerials handles GET /api/workorders/:id/serials.
func (h *Handler) WorkOrderSerials(w http.ResponseWriter, r *http.Request, id string) {
	var exists int
	err := h.DB.QueryRow("SELECT 1 FROM work_orders WHERE id=?", id).Scan(&exists)
	if err != nil {
		response.Err(w, "work order not found", 404)
		return
	}

	rows, err := h.DB.Query("SELECT id,wo_id,serial_number,status,COALESCE(notes,'') FROM wo_serials WHERE wo_id=? ORDER BY serial_number", id)
	if err != nil {
		response.Err(w, err.Error(), 500)
		return
	}
	...
	var serials []models.WOSerial
	for rows.Next() {
		var serial models.WOSerial
	...
	if serials == nil {
		serials = []models.WOSerial{}
	}

	response.JSON(w, serials)
}

// WorkOrderAddSerial handles POST /api/workorders/:id/serials.
func (h *Handler) WorkOrderAddSerial(w http.ResponseWriter, r *http.Request, id string) {
	var assemblyIPN string
	var woStatus string
	err := h.DB.QueryRow("SELECT assembly_ipn,status FROM work_orders WHERE id=?", id).Scan(&assemblyIPN, &woStatus)
	if err != nil {
		response.Err(w, "work order not found", 404)
		return
	}
	...
	if woStatus == "completed" || woStatus == "cancelled" {
		response.Err(w, "cannot add serials to completed or cancelled work order", 400)
		return
	}

	var serial models.WOSerial
	if err := response.DecodeBody(r, &serial); err != nil {
		response.Err(w, "invalid body", 400)
		return
	}
	...
	var count int
	h.DB.QueryRow("SELECT COUNT(*) FROM wo_serials WHERE serial_number=?", serial.SerialNumber).Scan(&count)
	if count > 0 {
	...
	serial.WOID = id
	if serial.Status == "" {
		serial.Status = "building"
	}

	_, err = h.DB.Exec("INSERT INTO wo_serials (wo_id,serial_number,status,notes) VALUES (?,?,?,?)",
	...
	audit.LogAudit(h.DB, h.Hub, audit.GetUsername(h.DB, r), "created", "serial", serial.SerialNumber, "Added serial "+serial.SerialNumber+" to WO "+id)

	err = h.DB.QueryRow("SELECT id,wo_id,serial_number,status,COALESCE(notes,'') FROM wo_serials WHERE wo_id=? AND serial_number=?",
	...
	response.JSON(w, serial)
}

// GenerateSerialNumber creates a serial number from the assembly IPN prefix and a timestamp.
