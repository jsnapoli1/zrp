...
func TestCAPACRUD(t *testing.T) {
	...
	req := testutil.AuthedRequest("GET", "/api/v1/capas", nil, cookie)
	h.ListCAPAs(w, req)
	if w.Code != 200 {
		t.Fatalf("list: expected 200, got %d", w.Code)
	}
	list, _ := unmarshalResp[[]models.CAPA](w.Body.Bytes())
	if len(list) != 0 {
	...
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("POST", "/api/v1/capas", []byte(body), cookie)
	h.CreateCAPA(w, req)
	if w.Code != 200 {
		t.Fatalf("create: expected 200, got %d: %s", w.Code, w.Body.String())
	}
	created, _ := unmarshalResp[models.CAPA](w.Body.Bytes())
	if created.ID == "" {
	...
	// Get
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("GET", "/api/v1/capas/"+created.ID, nil, cookie)
	h.GetCAPA(w, req, created.ID)
	if w.Code != 200 {
		t.Fatalf("get: expected 200, got %d", w.Code)
	}
	fetched, _ := unmarshalResp[models.CAPA](w.Body.Bytes())
	...
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("PUT", "/api/v1/capas/"+created.ID, []byte(updateBody), cookie)
	h.UpdateCAPA(w, req, created.ID)
	if w.Code != 200 {
		t.Fatalf("update: expected 200, got %d: %s", w.Code, w.Body.String())
	}
	updated, _ := unmarshalResp[models.CAPA](w.Body.Bytes())
...
func TestCAPACloseRequiresEffectivenessAndApproval(t *testing.T) {
	...
	// Create
	w := httptest.NewRecorder()
	req := testutil.AuthedRequest("POST", "/api/v1/capas", []byte(`{"title":"Test close","type":"corrective","owner":"eng"}`), cookie)
	h.CreateCAPA(w, req)
	c, _ := unmarshalResp[models.CAPA](w.Body.Bytes())

	// Close without effectiveness
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("PUT", "/api/v1/capas/"+c.ID, []byte(`{"title":"Test close","type":"corrective","owner":"eng","status":"closed"}`), cookie)
	h.UpdateCAPA(w, req, c.ID)
	if w.Code != 400 {
	...
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("PUT", "/api/v1/capas/"+c.ID, []byte(`{"title":"Test close","type":"corrective","owner":"eng","status":"closed","effectiveness_check":"Verified"}`), cookie)
	h.UpdateCAPA(w, req, c.ID)
	if w.Code != 400 {
	...
	w = httptest.NewRecorder()
	req = testutil.AuthedRequest("PUT", "/api/v1/capas/"+c.ID, []byte(`{"title":"Test close","type":"corrective","owner":"eng","status":"closed","effectiveness_check":"Verified OK","approved_by_qe":"QE Approved","approved_by_mgr":"Manager Approved"}`), cookie)
	h.UpdateCAPA(w, req, c.ID)
	if w.Code != 200 {
		t.Fatalf("expected 200, got %d: %s", w.Code, w.Body.String())
	}
	closed, _ := unmarshalResp[models.CAPA](w.Body.Bytes())
