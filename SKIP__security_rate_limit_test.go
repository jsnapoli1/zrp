...
func TestRateLimit_LoginEndpoint(t *testing.T) {
	...
	db.Exec("INSERT INTO users (username, password, role, active) VALUES (?, ?, ?, ?)",
		"testuser", passwordHash, "user", 1)
...
func TestRateLimit_GlobalAPILimit(t *testing.T) {
	...
	for i := 1; i <= 105; i++ {
		...
		req.Header.Set("Cookie", fmt.Sprintf("zrp_session=%s", token))
...
func TestRateLimit_PerEndpointLimits(t *testing.T) {
	...
	apiReq.Header.Set("Cookie", fmt.Sprintf("zrp_session=%s", token))
