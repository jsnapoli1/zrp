...
type SearchFilter struct {
	...
	Operator string      `json:"operator"` // eq, ne, contains, startswith, endswith, gt, lt, gte, lte, in, between
	Value    interface{} `json:"value"`
	AndOr    string      `json:"andOr"` // AND or OR (for combining with next filter)
}
...
type SavedSearch struct {
	...
	EntityType  string         `json:"entity_type"` // parts, workorders, ecos, inventory, etc.
	Filters     []SearchFilter `json:"filters"`
	SortBy      string         `json:"sort_by"`
	SortOrder   string         `json:"sort_order"`
	CreatedBy   string         `json:"created_by"`
	CreatedAt   time.Time      `json:"created_at"`
	IsPublic    bool           `json:"is_public"` // shared across team
}
...
type SearchQuery struct {
	...
	Filters     []SearchFilter `json:"filters"`
	SearchText  string         `json:"search_text"` // Multi-field text search
	SortBy      string         `json:"sort_by"`
...
func BuildSearchSQL(filters []SearchFilter, entityType string, searchText string) (string, []interface{}, error) {
	...
	// Add multi-field text search if provided
	if searchText != "" {
		...
		if len(textFilters) > 0 {
			...
			// For LIKE searches, add wildcards
...
// buildTextSearchFilters returns field list for multi-field search
...
func buildFilterClause(filter SearchFilter, argIndex *int) (string, []interface{}, error) {
	...
	case "contains", "like":
		// Support wildcards in the value
		pattern := valueStr
	...
	case "between":
		// Value should be an array with 2 elements [min, max]
		values, ok := filter.Value.([]interface{})
		if !ok || len(values) != 2 {
...
func GetQuickFilters(entityType string) []QuickFilter {
	...
	switch entityType {
	case "parts":
		filters = append(filters, QuickFilter{
			ID:         "active-parts",
			Name:       "Active Parts",
			EntityType: "parts",
			Filters: []SearchFilter{
				{Field: "status", Operator: "eq", Value: "active", AndOr: "AND"},
			},
		})
		filters = append(filters, QuickFilter{
			ID:         "obsolete-parts",
			...
			EntityType: "parts",
			Filters: []SearchFilter{
				{Field: "status", Operator: "eq", Value: "obsolete", AndOr: "AND"},
			},
		})
	...
	case "workorders":
		filters = append(filters, QuickFilter{
			...
			Filters: []SearchFilter{
				{Field: "status", Operator: "in", Value: []interface{}{"open", "in_progress"}, AndOr: "AND"},
			},
		})
		filters = append(filters, QuickFilter{
			ID:         "high-priority-wos",
			Name:       "High Priority",
			EntityType: "workorders",
			Filters: []SearchFilter{
	...
	case "inventory":
		filters = append(filters, QuickFilter{
			...
			Filters: []SearchFilter{
				{Field: "qty_on_hand", Operator: "lte", Value: "reorder_point", AndOr: "AND"},
			},
		})
		filters = append(filters, QuickFilter{
			ID:         "out-of-stock",
			...
			EntityType: "inventory",
			Filters: []SearchFilter{
				{Field: "qty_on_hand", Operator: "lte", Value: 0, AndOr: "AND"},
			},
		})
	...
	case "ecos":
		filters = append(filters, QuickFilter{
			...
			EntityType: "ecos",
			Filters: []SearchFilter{
				{Field: "status", Operator: "in", Value: []interface{}{"draft", "pending"}, AndOr: "AND"},
			},
		})
		filters = append(filters, QuickFilter{
			ID:         "approved-ecos",
			...
			EntityType: "ecos",
			Filters: []SearchFilter{
				{Field: "status", Operator: "eq", Value: "approved", AndOr: "AND"},
			},
		})
		filters = append(filters, QuickFilter{
			ID:         "high-priority-ecos",
			Name:       "High Priority",
			EntityType: "ecos",
			Filters: []SearchFilter{
				{Field: "priority", Operator: "eq", Value: "high", AndOr: "AND"},
...
func InitSearchTables(database *sql.DB) error {
	...
	// Create performance indexes on main entity tables for fast searching
	indexes := []string{
		// Parts indexes
...
// Supports: field:value, field>value, field<value, field:*value*, "exact phrase"
