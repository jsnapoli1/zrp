...
func TestPOAutogen_BOMShortageDetection(t *testing.T) {
	...
	handleGeneratePOSuggestions(w, req)
...
func TestPOAutogen_ApproveRejectWorkflow(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/v1/pos/suggestions/review", bytes.NewBufferString(reqBody))
	w := httptest.NewRecorder()

	handleReviewPOSuggestion(w, req, suggestionID)
	...
	req2 := httptest.NewRequest("POST", "/api/v1/pos/suggestions/review", bytes.NewBufferString(reqBody2))
	w2 := httptest.NewRecorder()

	handleReviewPOSuggestion(w2, req2, suggestionID2)
