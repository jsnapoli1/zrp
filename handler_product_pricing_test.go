...
func TestProductPricingCRUD(t *testing.T) {
	...
	req := authedRequest("POST", "/api/v1/pricing", []byte(body), loginAdmin(t, db))
	...
	req2 := authedRequest("POST", "/api/v1/pricing", []byte(body2), loginAdmin(t, db))
	...
	// List all pricing
	req3 := authedRequest("GET", "/api/v1/pricing", nil, loginAdmin(t, db))
	...
	// Get single pricing
	req4 := authedRequest("GET", "/api/v1/pricing/1", nil, loginAdmin(t, db))
	...
	req5 := authedRequest("PUT", "/api/v1/pricing/1", []byte(updateBody), loginAdmin(t, db))
	...
	// Delete pricing
	req6 := authedRequest("DELETE", "/api/v1/pricing/1", nil, loginAdmin(t, db))
	...
	// Verify deleted
	req7 := authedRequest("GET", "/api/v1/pricing/1", nil, loginAdmin(t, db))
...
func TestProductPricingAnalysis(t *testing.T) {
	...
	req := authedRequest("POST", "/api/v1/pricing", []byte(body), cookie)
	w := httptest.NewRecorder()
	handleCreateProductPricing(w, req)
	if w.Code != 200 {
	...
	req2 := authedRequest("POST", "/api/v1/pricing/analysis", []byte(costBody), cookie)
	w2 := httptest.NewRecorder()
	...
	// Get analysis list
	req3 := authedRequest("GET", "/api/v1/pricing/analysis", nil, cookie)
	w3 := httptest.NewRecorder()
...
func TestProductPricingHistory(t *testing.T) {
	...
	// Get history
	req := authedRequest("GET", "/api/v1/pricing/history/IPN-001", nil, cookie)
	w := httptest.NewRecorder()
...
func TestProductPricingBulkUpdate(t *testing.T) {
	...
	req := authedRequest("POST", "/api/v1/pricing/bulk-update", []byte(bulkBody), cookie)
	w := httptest.NewRecorder()
	...
	req2 := authedRequest("GET", "/api/v1/pricing/1", nil, cookie)
	w2 := httptest.NewRecorder()
