...
func setupAPIKeysTestDB(t *testing.T) *sql.DB {
...
func TestHandleToggleAPIKey_Enable(t *testing.T) {
	...
	reqBody := `{"enabled": 1}`
	req := httptest.NewRequest("PATCH", "/api/keys/"+string(rune(id+48)), bytes.NewBufferString(reqBody))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	handleToggleAPIKey(w, req, string(rune(id+48)))
...
func TestValidateBearerToken_NotFound(t *testing.T) {
	...
	// Try key that doesn't exist in DB
