...
func TestWorkOrderKitting_BasicReservation(t *testing.T) {
	...
	_, err = db.Exec(`INSERT INTO work_orders (id, assembly_ipn, qty, status, created_at) 
	...
	handleWorkOrderKit(w, req, "WO-001")
	...
	err = db.QueryRow("SELECT qty_on_hand, qty_reserved FROM inventory WHERE ipn='PART-001'").
...
func TestWorkOrderKitting_MultipleWOsCompetingInventory(t *testing.T) {
	...
	handleWorkOrderKit(w1, req1, "WO-100")
	...
	err = db.QueryRow("SELECT qty_reserved FROM inventory WHERE ipn='PART-002'").Scan(&reserved)
	if err != nil {
		t.Fatal(err)
	}
	if reserved != 5.0 {
	...
	handleWorkOrderKit(w2, req2, "WO-101")
...
func TestWorkOrderKitting_CompletionReleasesReservation(t *testing.T) {
	...
	// Update work order to complete status (DB schema uses "complete" not "completed")
	updateJSON := `{
	...
	w := httptest.NewRecorder()
	handleUpdateWorkOrder(w, req, "WO-200")
	...
	var onHand, reserved float64
	err = db.QueryRow("SELECT qty_on_hand, qty_reserved FROM inventory WHERE ipn='PART-003'").
		Scan(&onHand, &reserved)
	if err != nil {
		t.Fatal(err)
	}
	...
	if reserved != 0.0 {
		t.Errorf("Expected qty_reserved to be 0 after completion, got %f", reserved)
	}
...
func TestWorkOrderKitting_CancellationReleasesReservation(t *testing.T) {
	...
	w := httptest.NewRecorder()
	handleUpdateWorkOrder(w, req, "WO-300")
	...
	var onHand, reserved float64
	err = db.QueryRow("SELECT qty_on_hand, qty_reserved FROM inventory WHERE ipn='PART-004'").
		Scan(&onHand, &reserved)
	if err != nil {
		t.Fatal(err)
	}
	...
	if reserved != 0.0 {
		t.Errorf("Expected qty_reserved to be 0 after cancellation, got %f", reserved)
	}
	if onHand != 20.0 {
...
func TestWorkOrderKitting_ReservedInventoryNotAvailableForOtherWOs(t *testing.T) {
	...
	w := httptest.NewRecorder()
	handleWorkOrderKit(w, req, "WO-400")
	...
	var reserved float64
	err = db.QueryRow("SELECT qty_reserved FROM inventory WHERE ipn='PART-005'").Scan(&reserved)
	if err != nil {
		t.Fatal(err)
	}

	if reserved > 15.0 {
		t.Errorf("Reserved quantity should not exceed on_hand (15), got %f", reserved)
	}
}
...
func TestWorkOrderKitting_SecondWOProceedsAfterFirstCompletes(t *testing.T) {
	...
	w1 := httptest.NewRecorder()
	handleWorkOrderKit(w1, req1, "WO-500")
	...
	w2 := httptest.NewRecorder()
	handleWorkOrderKit(w2, req2, "WO-501")
	...
	// Step 4: Complete WO-1 (DB schema uses "complete" not "completed")
	updateJSON := `{
	...
	handleUpdateWorkOrder(w3, req3, "WO-500")
	...
	err = db.QueryRow("SELECT qty_on_hand, qty_reserved FROM inventory WHERE ipn='PART-006'").
	...
	_, err = db.Exec(`UPDATE inventory SET qty_on_hand = 15.0 WHERE ipn='PART-006'`)
	if err != nil {
		t.Fatal(err)
	}
	...
	handleWorkOrderKit(w4, req4, "WO-501")
