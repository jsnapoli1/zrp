...
// handleCreateCAPAFromNCR creates a CAPA directly from an NCR (Gap 5.1)
...
func handleCreateCAPAFromNCR(w http.ResponseWriter, r *http.Request, ncrID string) {
	...
	var ncr NCR
	err := db.QueryRow(`SELECT id,title,COALESCE(description,''),COALESCE(ipn,''),COALESCE(serial_number,''),
	...
	if r.ContentLength > 0 {
		if err := decodeBody(r, &requestData); err != nil {
	...
	_, err = db.Exec(`INSERT INTO capas (id,title,type,linked_ncr_id,root_cause,action_plan,owner,due_date,
...
// handleCreateECOFromNCR creates an ECO directly from an NCR (Gap 5.7)
...
func handleCreateECOFromNCR(w http.ResponseWriter, r *http.Request, ncrID string) {
	...
	_, err = db.Exec(`INSERT INTO ecos (id,title,description,status,priority,affected_ipns,created_by,
	...
	logAudit(db, username, "created", "eco", ecoID, fmt.Sprintf("Created ECO from NCR %s", ncrID))
	recordChangeJSON(username, "ecos", ecoID, "create", nil, newECO)
